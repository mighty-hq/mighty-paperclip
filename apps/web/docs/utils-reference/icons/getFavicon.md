...
Returns an [Image.ImageLike](../../api-reference/user-interface/icons-and-images.md) that can be used where mighty expects them.

## Example

```tsx
import { List } from "@mighty/api";
import { getFavicon } from "@mighty/utils";

export default function Command() {
  return (
    <List>
      <List.Item icon={getFavicon("https://mighty.com")} title="mighty Website" />
    </List>
  );
}
```
