...
- `pagination` - the pagination object that mighty [`List`s](https://developers.mighty.com/api-reference/user-interface/list#props) and [`Grid`s](https://developers.mighty.com/api-reference/user-interface/grid#props) expect.
...
```ts
import { Action, ActionPanel, List, environment } from "@mighty/api";
import { useStreamJSON } from "@mighty/utils";
