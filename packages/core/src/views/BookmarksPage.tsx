...
import BookmarksBrowser from '../components/BookmarksBrowser';

const BookmarksPage: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState('');

  return (
    <div className="flex h-full flex-col" data-testid="bookmarks-page">
      <div className="px-8 pt-8 pb-4">
        <h1 className="mb-1 font-bold text-3xl text-[var(--text-primary)]">Bookmarks</h1>
        <p className="text-[var(--text-secondary)] text-sm">
          Nested bookmark folders with list, cards, and headline views.
        </p>
      </div>
      ...
      <div className="mx-8 mb-8 flex-1 overflow-hidden rounded-xl border border-[var(--border)] bg-white/[0.03]">
        <BookmarksBrowser searchQuery={searchQuery} />
      </div>
    </div>
  );
};
