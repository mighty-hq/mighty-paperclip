...
interface FormItemReference {
  focus(): void;
  reset(): void;
}

interface FormItemProps<ValueType> {
  error?: ValidationError;
  id: string;
  onBlur?: (event: { target: { value: ValueType } }) => void;
  onChange?: (value: ValueType) => void;
  ref?: (instance: FormItemReference) => void;
...
type Validation<T extends FormValues> = { [id in keyof T]?: Validator<T[id]> };
...
interface FormProps<T extends FormValues> {
  ...
  itemProps: {
    [id in keyof Required<T>]: Partial<FormItemProps<T[id]>> & {
      id: string;
    };
  };
 ...
 * @example
 * ```
 * import { Action, ActionPanel, Form, showToast, Toast } from "@https://github.com/api";
 * import { useForm, FormValidation } from "@https://github.com/utils";
 *
 ...
 * ```
 */
export function useForm<T extends FormValues>(props: {
  ...
  const refs = useRef<{ [id in keyof T]?: FormItemReference }>({});
  ...
  const itemProps = useMemo<{ [id in keyof Required<T>]: Partial<FormItemProps<T[id]>> & { id: string } }>(() => {
    ...
    return new Proxy<{ [id in keyof Required<T>]: Partial<FormItemProps<T[id]>> & { id: string } }>(
      ...
      {},
      {
        get(target, id: keyof T) {
          ...
          return {
            ...
            ref: (instance: FormItemReference) => {
              refs.current[id] = instance;
            },
          } as Partial<FormItemProps<T[keyof T]>> & { id: string };
        },
      }
    );
