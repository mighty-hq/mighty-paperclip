...
import { environment, LaunchType, Toast } from '@mighty/api';
 ...
 * import { List } from "@https://github.com/api";
 * import { usePromise } from "@https://github.com/utils";
 *
    ...
    async (asyncUpdate, options) => {
      ...
      try {
        ...
        if (options?.shouldRevalidateAfter !== false) {
          if (environment.launchType === 'background' || environment.commandMode === 'menu-bar') {
